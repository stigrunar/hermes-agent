...
| `bws authentication failed` | Token revoked or wrong | Generate a new token, re-run setup |
| `bws authentication failed for the configured Bitwarden region` | Token is for a different Bitwarden region (e.g. an EU token against the US endpoint) | Re-run setup and pick the right region, or set `secrets.bitwarden.server_url` to `https://vault.bitwarden.eu` (or your self-hosted URL) |
