...
export const zhHant = defineLocale({
  ...
  settings: {
    ...
    kanban: {
      ...
      boardsLoadFailed: '看板載入失敗',
      boardLoadFailed: '看板載入失敗',
      unavailableTitle: '看板無法使用',
      unavailableDescription: '此設定檔未啟用看板外掛。',
      loadFailedTitle: '無法載入看板',
      loadFailedDescription: '請檢查後端連線後再試一次。',
      retry: '重試',
      refresh: '重新整理',
      boardCount: count => `${count} 個看板`,
      taskCount: count => `${count} 個任務`,
      current: '目前',
      emptyTitle: '沒有看板',
      emptyDescription: '目前設定檔中沒有看板。',
      loadingBoard: '正在載入看板…',
      noSelectionTitle: '尚未選擇看板',
      noSelectionDescription: '請從左側選擇一個看板。',
      boardUnavailableTitle: '看板無法使用',
      boardUnavailableDescription: '無法載入此看板。請選擇其他看板或再試一次。',
      untitledTask: '未命名任務',
      moreTasks: count => `另有 ${count} 個`
    },
    notifications: {
      title: '通知',
    ...
    gateway: {
      ...
      profileConnection: profile => `僅當「${profile}」為作用中設定檔時使用此連線。選擇使用預設即可繼承全域連線。`,
      useDefaultTitle: '使用預設連線',
      useDefaultDesc: '繼承「全部設定檔」中選擇的連線。',
