# Hermes Agent — Contextual Development Reference

This file contains detailed development procedures and subsystem references
for contributors and coding agents. The root [`AGENTS.md`](../../AGENTS.md)
owns the always-read repository identity, downstream execution boundaries,
contribution rubric, and the task triggers that select sections here.

Do not read this entire file for every change. Start at the heading named by
the root trigger, then follow only dependencies revealed by the task.
