...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if stored_prompt and _stored_prompt_matches_runtime(agent, stored_prompt):
        # Ordinary long-lived sessions need the same freshness guarantee as Bot
        # Chat, without inheriting Bot Mode's messaging protocol. The source
        # epoch is stamped in the volatile prompt tier and changes only at a
        # deliberate rebuild boundary, preserving exact prompt reuse otherwise.
        ...
        try:
            ...
            # A probe failure must fail closed to prompt reuse; cache churn is
            # preferable only when a source change is actually observed.
            _prompt_source_stale = False
