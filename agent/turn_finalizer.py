...
def _record_kanban_budget_exhausted(
    ...
    Routed via the dedicated iteration-exhaustion transition (not ``kanban_block``),
    which records the terminal timeout without pretending the worker explicitly
    blocked the task. The transition is run-fenced and idempotent, so it is safe
    ...
    try:
        ...
        try:
            ...
            # Bind the terminal transition to the worker run that exhausted its
            # budget; a reclaimed worker must not close its successor run.
