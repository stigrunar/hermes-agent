...
def _codex_runtime_contract(agent, messages, cwd):
    ...
    # Generic subprocesses deliberately lose dispatcher write authority. This
    # MCP callback is the worker's own tool surface, so restore that identity
    # only while the current in-process execution is the dispatcher-owned
...
def _codex_thread_binding(agent, cwd):
    ...
    if db is None or not isinstance(session_id, str) or not session_id:
        return None, None
    key = "codex_app_server.thread:" + session_id
    identity = {
    ...
    if isinstance(saved, str) and saved:
        ...
        if not isinstance(record, dict) or any(record.get(field) != value for field, value in identity.items()):
    ...
    def persist(new_thread_id):
        db.set_meta(key, json.dumps({**identity, "thread_id": new_thread_id}, sort_keys=True))
...
def _ensure_codex_session(agent) -> None:
    ...
    contract, _ = _codex_runtime_contract(agent, [], cwd)
