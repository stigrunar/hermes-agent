...
KANBAN_GUIDANCE = (
    ...
    "ground truth.\n"
    "2. **Accept specialist ownership before work.** If you are a specialist "
    "profile (anything other than `default`) and the task thread does not "
    "already contain formal acceptance, your first write action after "
    "orientation must be `kanban_comment` with: `accepted_by`, `accepted_at`, "
    "`lane`, `scope_understood`, `first_action`, `expected_artifact`, "
    "`risk_level`, and `will_not_do`. Do this before editing files, running "
    "deploys, or creating child work.\n"
