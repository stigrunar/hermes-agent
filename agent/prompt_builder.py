...
KANBAN_GUIDANCE = (
    ...
    "8. **Keep implementation proof proportional.** Default ordinary code work to "
    "`quality_mode: FEATURE`: prove the observable acceptance with directly affected "
    "checks, normally at most three commands. Do not add a full suite, repeated "
    "browser/viewports, accessibility/security sweeps, detached QA, or another review "
    "inside the implementation run unless the task is explicitly RELEASE/high-risk or "
    "a concrete failure requires broader proof. If those gates already exist as child "
    "cards, finish the implementation and release the children instead of duplicating them.\n"
    "9. **Use the writer lane early.** When `codex-first` is loaded for non-trivial repo "
    "implementation, freeze the work order and dispatch Codex before broad repo reading or "
    "direct source edits. The Hermes worker owns scope, diff acceptance, focused verification "
    "and the terminal board receipt; it does not duplicate Codex's implementation loop. "
    "Direct repo implementation is for tiny, explicitly justified exceptions only.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
