...
KANBAN_GUIDANCE = (
    ...
    "ground truth.\n"
    "2. **Accept or block before work.** After `kanban_show()`, either call "
    "`kanban_block(reason=...)` and stop, or immediately call `kanban_comment()` "
    "with an acceptance note containing all fields on separate `key: value` lines: "
    "`accepted_by`, `accepted_at`, `lane`, `scope_understood`, `first_action`, "
    "`expected_artifact`, `risk_level`, and `will_not_do`. Do not start repo, "
    "ops, private, or research work until this acceptance comment is written.\n"
    "3. **Work inside the workspace.** `cd $HERMES_KANBAN_WORKSPACE` before "
    ...
    "5. **Block on genuine ambiguity.** If you need a human decision you cannot "
    "infer (missing credentials, UX choice, paywalled source, peer output you "
    "need first), call `kanban_block(reason=\"...\")` and stop. Don't guess. "
    "The user will unblock with context and the dispatcher will respawn you.\n"
    "6. **Refresh comments before finishing.** Call `kanban_show()` again before "
    "`kanban_complete()` or `kanban_block()` if meaningful time/tool work has "
    "passed, so late Matrix/Paperclip-style comments are included in your final "
    "decision.\n"
    ...
    "scope-creeping into the next thing. A task may be `done` only for its own "
    "scoped acceptance criteria/artifact; if follow-up remains, keep it as a "
    "linked open child/backlog/blocker and report `slice done; follow-up open`, "
    "never project-level done.\n"
    "9. **Specialist ownership acceptance.** A task assigned or handed off to a "
    "specialist is not `in_progress` / `i arbeid` until that specialist records "
    "explicit acceptance fields: `accepted_by`, `accepted_at`, `lane`, "
    "`scope_understood`, `first_action`, `expected_artifact`, `risk_level`, "
    "and `will_not_do`. If any field is missing, keep ownership with the "
    "orchestrator/router and do not report the specialist work as underway.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
