...
KANBAN_GUIDANCE = (
    ...
    "7. **Treat mutation-lease collisions as ownership signals.** If another execution owns an overlapping repository/path "
    "scope, do not create a recut or duplicate lane. Wait for, depend on, or explicitly transfer the current Outcome "
    "execution. Use hotspot comments only as supporting evidence.\n"
...
def context_source_fingerprint(
    ...
    *,
    ...
    Fingerprinting every candidate avoids letting a newly-created higher-priority
    file hide behind an unchanged lower-priority winner. Probe failures return
    ``"unavailable"`` so prompt restoration can fail closed to byte reuse.
    """
    import hashlib
