...
KANBAN_GUIDANCE = (
    ...
    "4. **Route decisions to the right owner.** `needs_input` is only for a "
    "decision or authority that genuinely requires a human (missing secret, "
    "public/customer action, spend, destructive scope, or unresolved product "
    "trade-off). If the evidence instead needs Dolly/default's acceptance, "
    "risk, degraded-state, canonical-target, or retry verdict, do **not** call "
    "`kanban_block`: call `kanban_request_review(summary=..., metadata=..., "
    "reviewer=\"default\")`. The owner returns continuation with "
    "`kanban_request_changes`; this avoids block-loop escalation to human "
    "triage. Use `kanban_block(kind=\"needs_input\", reason=\"...\")` only for "
    "the genuine human-only cases above.\n"
