...
KANBAN_GUIDANCE = (
    ...
    "6. **Do not manufacture a follow-up graph.** If follow-up work stays inside "
    "the same Project/Outcome, authority, and mutation scope, keep the lifecycle "
    "coherent: finish the current bounded execution or record the next action on "
    "the same Outcome. Create a child task only for a real independent scope, "
    "cross-authority handoff, durable wait, or required independent gate. Any "
    "mutating child must preserve `project_id`, `outcome_id`, canonical base, and "
    "a non-overlapping `mutation_scope`; a conversation/topic is never authority.\n"
    "7. **Treat mutation-lease collisions as ownership signals.** If another "
    "execution owns an overlapping repository/path scope, do not create a recut "
    "or duplicate lane. Wait for, depend on, or explicitly transfer the current "
    "Outcome execution. Use hotspot comments only as supporting evidence.\n"
    "8. **Flag collision hotspots; don't pile on.** If your change keeps "
    ...
    "## Orchestrator mode\n"
    "\n"
    "Only enter decomposition mode when the task explicitly asks for decomposition "
    "and has already passed the Project/Outcome admission boundary. Prefer one "
    "coherent execution. Fan out with `kanban_create` only for genuinely "
    "independent non-overlapping scopes or distinct authorities, and propagate "
    "the owning Project/Outcome identity into every child. Do not decompose an "
    "ordinary FEATURE merely because multiple specialist profiles exist.\n"
    "\n"
