...
KANBAN_GUIDANCE = (
    ...
    "8. **Keep implementation proof proportional.** Ordinary FEATURE code owns directly "
    "affected deterministic checks and, only when visible behavior cannot otherwise be "
    "proved, at most one representative local browser smoke. Full viewport matrices, "
    "actual-target journeys, accessibility/security sweeps and release proof belong to an "
    "existing QA/release gate; never duplicate them inside Code.\n"
    "9. **Execute according to shape.** DollyCode is the Luna/xhigh leaf and normally "
    "implements its bounded packet directly. A default/Sol controller with `codex-first` "
    "dispatches native Codex v2 early and owns integration; do not create a Luna -> Sol -> "
    "Luna wrapper for leaf work.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
    "Classify before creating implementation work. `execution_shape: leaf` means one "
    "behavior change, one coherent mutation cluster, frozen decisions and focused proof; "
    "assign it to DollyCode and include `mutation_scope`, `acceptance_commands` and "
    "`qa_boundary`. `execution_shape: controller` means cross-module/source integration, "
    "multiple mutable scopes or multi-phase acceptance; keep one durable card with "
    "default/Sol, which uses `codex-first` and native Sol -> Luna v2 packages. Create "
    "separate Kanban cards only for genuinely independent durable scopes, named specialists "
    "or real QA/deploy gates — never send the whole mission to DollyCode or micro-slice a "
    "coherent outcome into ceremony.\n"
    "\n"
