...
def resolve_anthropic_token() -> Optional[str]:
    ...
    def _read_creds() -> Optional[Dict[str, Any]]:
        ...
        if not creds_loaded:
            creds = (
                None
                if (scoped or is_multiplex_active())
    ...
    # 4. Claude Code credential file. ``None`` is an intentional sentinel here
    # when multiplexing is active: do not let the helper re-read host-global
    # Claude Code files outside the active profile's secret scope.
    scoped_creds = _read_creds()
    resolved_claude_token = (
        _resolve_claude_code_token_from_credentials(scoped_creds)
        if scoped_creds is not None
        else None
    )
