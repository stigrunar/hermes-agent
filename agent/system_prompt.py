...
def prompt_source_fingerprint(agent: Any) -> str:
    ...
    The profile-owned surface is intentionally read-only and cheap: SOUL and
    config bytes, skills snapshot/generation markers, and project-context
    candidates visible from the session cwd. Runtime identity is checked
    separately by ``_stored_prompt_matches_runtime``. Probe failures return
    ``"unavailable"`` so restore fails closed to the persisted prompt.
    """
    import hashlib
    ...
    try:
        ...
        try:
            stat = snapshot.stat()
            profile_rows.append(f"skills_snapshot:{stat.st_mtime_ns}:{stat.st_size}")
        ...
        try:
            profile_rows.append(f"skills_root:{skills_root.stat().st_mtime_ns}")
        except FileNotFoundError:
        ...
        payload = (
            "v2|" + "|".join(profile_rows)
            + f"|project={project_context}|skip_context={bool(getattr(agent, 'skip_context_files', False))}"
        ).encode("utf-8")
...
def stored_prompt_source_epoch_needs_refresh(agent: Any, prompt: str) -> bool:
    ...
    A stamped prompt rebuilds only when the current fingerprint differs. Legacy
    prompts are migrated once for persistent gateway sessions; CLI/TUI resumes
    retain their historical no-stamp behavior. Probe failure returns ``False``.
    """
    try:
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Ordinary long-lived gateway sessions preserve their prompt for cache
    # efficiency. Stamp the profile/project source surface so a later change
    # can rebuild this same session exactly once. Bot Chat's capability epoch
    # remains separate and is added by ``_bot_mode_parts``.
...
__all__ = ["build_system_prompt_parts", "build_system_prompt", "invalidate_system_prompt",
    "platform_hint", "restore_plugin_prompt_sections", "format_tools_for_system_message",
           "prompt_source_fingerprint", "prompt_source_epoch_line",
           "stored_prompt_source_epoch_needs_refresh"]
