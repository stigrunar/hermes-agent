...
def _(rid, params, pdb, conn) -> dict:
    ...
    with odb.connect_closing() as outcomes_conn:
        ...
        required_project_id = str(
            params.get("depends_on_project_id") or proj.id).strip()
        ...
        if required is None:
            raise ValueError(
                "required Outcome does not resolve inside depends_on_project_id")
        ...
        dependency = next(
            item for item in odb.list_outcome_dependencies(
                outcomes_conn, outcome_id=outcome.id)
