...
class TestExcludeSources:
    ...
    def test_list_sessions_rich_keeps_excluded_source_with_matching_origin(self, db):
        ...
        sessions = db.list_sessions_rich(exclude_sources=["tui"], include_origin_sources=["telegram"])

        assert [session["id"] for session in sessions] == ["local"]
        assert db.session_count(exclude_sources=["tui"], include_origin_sources=["telegram"]) == 1
        assert [
            session["id"]
            for session in db.list_sessions_rich(source="telegram", include_origin_sources=["telegram"])
        ] == ["local"]
