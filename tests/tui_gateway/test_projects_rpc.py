...
def test_project_tree_row_preserves_messaging_metadata():
    row = server._project_tree_row(
        {
            "id": "s1",
            "chat_id": "chat",
            "chat_type": "group",
    ...
    assert {key: row[key] for key in ("chat_id", "chat_type", "thread_id", "display_name", "origin_json")} == {
        "chat_id": "chat",
        "chat_type": "group",
        "thread_id": "topic",
        "display_name": "Team",
