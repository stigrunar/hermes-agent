...
async def _shutdown_test_executor():
    """Let current upstream's gateway import release ``to_thread`` workers."""
    yield
...
async def test_stopping_gateway_does_not_spawn_late_drain():
    ...
    assert runner._schedule_telegram_deferred_delivery(profile="default") is None
    assert runner._telegram_deferred_drains == {}
