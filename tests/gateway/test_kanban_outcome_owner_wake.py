    ...
    *,
    ...
    with kbc.connect_closing() as conn:
        task_id = kb.create_task(
            ...
            conversation_lane_id=lane_id if task_lane_bound else None,
        )
...
class _OwnerAdapter:
    ...
    def __init__(self, *, fail_once: bool = False):
        ...
        self.handled = []
        self.sent = []
        self._message_handler = self.handle_message
...
def test_replay_after_outcome_revision_change_does_not_rewake_owner(tmp_path, monkeypatch):
    ...
    replay = resolve_outcome_owner_wake_spec("hermes", task, event)
...
def test_bound_event_keeps_passive_origin_notifications(tmp_path, monkeypatch):
    ...
    asyncio.run(runner._kanban_notifier_watcher(interval=0))
...
def test_stale_revision_and_human_gate_fail_closed(tmp_path, monkeypatch):
    _, outcome_id, _, task, event = _bound_event(tmp_path, monkeypatch)
    stale = resolve_outcome_owner_wake_spec("hermes", task, event)
    ...
    gated = resolve_outcome_owner_wake_spec("hermes", task, event)
