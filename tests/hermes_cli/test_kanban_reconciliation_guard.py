...
def test_dry_run_is_byte_and_semantic_no_write_even_for_maintenance(
    ...
    with kb.connect() as conn:
        ...
        monkeypatch.setattr(
            ...
            "_worker_scope_state",
            lambda *_a, **_k: (_ for _ in ()).throw(
                AssertionError("dry-run probed systemd worker scope")
            ),
        )
        monkeypatch.setattr(
            kb,
            "_pid_alive",
            lambda *_a, **_k: (_ for _ in ()).throw(
                AssertionError("dry-run probed host process liveness")
            ),
        )
        ...
        # Crash state is unknowable without a host-process probe; preview
        # conservatively retains recorded-PID claims.
