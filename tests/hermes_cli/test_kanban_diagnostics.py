...
def test_typed_reconciliation_serialization_redacts_private_diagnostics():
    hostile = (
        "https://alice:supersecret@example.invalid/repo?token=opaque-secret "
        "Authorization: Bearer sk-proj-1234567890abcdef "
        "path=/home/alice/private /run/user/1000/bus"
    )
    report = kd.ReconciliationResult(
        task_id="t_safe",
        candidate={"repo": hostile, "error": hostile},
        review={"error": hostile},
        replacement={"workspace": hostile},
        findings=[
            kd.Diagnostic(
                kind="probe_error",
                severity="error",
