...
def test_legacy_text_pk_tables_rebuilt_to_integer_autoincrement(tmp_path, monkeypatch):
    ...
    with kb.connect(db_path) as conn:
        ...
        # The non-numeric legacy cursor cannot be preserved, so migration
        # baselines the row at the current task boundary instead of replaying
        # historical events.
        sub = conn.execute(
...
def test_current_schema_legacy_rows_are_baselined_once(tmp_path, monkeypatch):
    """Legacy rows skip history once, then deliver events appended later."""
    db_path = _setup_home(tmp_path, monkeypatch)
    with kb.connect(db_path) as conn:
        ...
        conn.execute("CREATE INDEX idx_notify_task ON kanban_notify_subs(task_id)")
...
def test_legacy_nonzero_cursor_preserves_cursor_without_replaying_history(
    tmp_path, monkeypatch,
):
    """A nonzero legacy cursor stays put while the baseline suppresses history."""
    ...
    with kb.connect() as conn:
        ...
        assert sub["baseline_event_id"] == baseline_event
        cursor, events = kb.unseen_events_for_sub(
            conn,
            task_id=task_id,
            platform="telegram",
        ...
        assert cursor == baseline_event
        assert events == []

        kb._append_event(conn, task_id, kind="timed_out")
        future_event = conn.execute(
            "SELECT MAX(id) FROM task_events WHERE task_id=?", (task_id,)
        ).fetchone()[0]
        cursor, events = kb.unseen_events_for_sub(
            conn,
        ...
        assert cursor == future_event
        assert [event.id for event in events] == [future_event]
        assert [event.kind for event in events] == ["timed_out"]
