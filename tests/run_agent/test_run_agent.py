...
class TestRunConversation:
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        """Kanban iteration exhaustion must terminalize the exact run once.

        It is a concrete end reason, not a generic timeout or spawn failure, so
        it bypasses the automatic retry circuit and preserves the checkpoint.
        """
        self._setup_agent(agent)
        ...
        mock_record_exhaustion = MagicMock(return_value=7)
        ...
        assert call.kwargs["budget_used"] == 2
        assert call.kwargs["budget_max"] == 2
