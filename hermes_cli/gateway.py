...
def _gateway_command_inner(args):
    ...
    if subcmd == "recovery-guard":
        from hermes_cli.recovery_guard import GuardError, arm

        try:
            run_dir = arm(Path(args.plan))
        except GuardError as exc:
            print_error(f"Could not arm recovery guard: {exc}")
            raise SystemExit(2) from exc
        print_success(f"Recovery guard armed: {run_dir}")
        print_info(f"Durable receipt: {run_dir / 'result.json'}")
        return
