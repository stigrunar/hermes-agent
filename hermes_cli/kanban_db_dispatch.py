...
def enforce_max_runtime(conn: sqlite3.Connection, *, signal_fn=None) -> list[str]:
    ...
    for row in rows:
        ...
        if not scope_release.can_release:
            # Do not clear a timed-out claim until the exact worker scope has
            # been stopped and its cgroup absence proven.
            continue
        # SIGTERM then SIGKILL after 5 s grace; workers wanting a cleaner
        # shutdown install their own SIGTERM handler.
        killed = False
        kill = _kill_fn(signal_fn)
        if kill is not None and scope_release.pid_signal_allowed:
...
def detect_stale_running(
    ...
    *,
    ...
    for row in rows:
        ...
        termination = (
            _kb._terminate_reclaimed_worker(pid, lock, signal_fn=signal_fn)
            if scope_release.pid_signal_allowed
            else _kb._termination_metadata_without_pid_signal(pid, scope_release)
        )
...
def reconcile_orphaned_running(conn: sqlite3.Connection) -> list[str]:
    ...
    for row in rows:
        ...
        if pid and pid_signal_allowed and _kb._pid_alive(pid):
...
def _classify_dead_worker(pid: int, claimer: Optional[str]) -> _DeadWorker:
    ...
    # Resolve through the facade so callers that own the public kanban_db seam
    # can provide the authenticated exit classification during recovery.
...
def _reclaim_dead_workers(conn: sqlite3.Connection) -> _CrashSweep:
    ...
    with _kb.write_txn(conn):
        ...
        for row in rows:
            ...
            if scope_release.pid_signal_allowed and _kb._pid_alive(row["worker_pid"]):
                continue
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    """Persist a worker PID and its authenticated launch receipt."""
    ...
    with _kb.write_txn(conn):
        ...
        cur = conn.execute(
            "UPDATE tasks SET worker_pid=? WHERE id=? AND status='running' AND current_run_id=?",
            (int(pid), task_id, run_id),
        )
        if cur.rowcount != 1:
            raise RuntimeError("worker launch lost its task/run fence")
        cur = conn.execute(
            "UPDATE task_runs SET worker_pid=?, launch_mode=?, scope_unit=?, manager_kind=?, manager_uid=?, "
            ...
            (int(pid), launch_mode, scope_unit, manager_kind, manager_uid,
...
def _default_spawn(
    ...
    *,
    ...
    worker_cmd = _worker_argv(task, profile_arg, env.get("HERMES_HOME"))
