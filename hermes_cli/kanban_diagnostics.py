...
def _terminal_active_run_diagnostic(
    ...
    """Report Phase-B terminal/reaping state without exposing run payloads."""
    ...
    return Diagnostic(
        ...
        last_seen_at=now,
        data={"status": status, "run_id": run_id, "worker_pid": worker_pid},
    )
...
def _event_history_queries(placeholders: str) -> tuple[tuple[str, tuple[Any, ...]], ...]:
    ...
    return (
        ...
        (
            f"""
            SELECT e.* FROM task_events e
                 INDEXED BY idx_events_task_kind_id
            WHERE e.task_id IN ({placeholders})
              AND e.kind IN (
                  'review_gate_invalid_workspace',
                  'review_gate_repaired',
                  'review_gate_replaced'
              )
            """,
            (),
        ),
...
def _run_history_queries(placeholders: str) -> tuple[str, ...]:
    return (
        ...
        JOIN task_runs r ON r.id = (
              SELECT MAX(latest.id)
              FROM task_runs latest INDEXED BY idx_runs_task_outcome_id
              WHERE latest.task_id = task.id
        ...
        SELECT r.*
        FROM tasks task
        JOIN task_runs r INDEXED BY idx_runs_task_outcome_id
          ON r.task_id = task.id
         AND r.outcome = 'crashed'
         AND r.id > COALESCE((
              SELECT MAX(breaker.id)
              FROM task_runs breaker INDEXED BY idx_runs_task_outcome_id
        ...
        SELECT r.*
        FROM tasks task
        JOIN task_runs r ON r.id = task.current_run_id
        WHERE task.id IN ({placeholders})
        """,
        f"""
        SELECT r.*
        ...
        JOIN task_runs r ON r.id = (
              SELECT candidate.id
              FROM task_runs candidate INDEXED BY idx_runs_task
              WHERE candidate.task_id = task.id
                AND candidate.reap_state IN (
                    'terminal_requested', 'reap_pending', 'reaping',
                    'identity_unverifiable', 'manual_recovery_required', 'gave_up'
                )
                AND NOT (
                    candidate.reap_state = 'manual_recovery_required'
                    AND candidate.status = 'reclaimed'
                    AND candidate.outcome = 'reclaimed'
                    AND candidate.ended_at IS NOT NULL
                    AND candidate.worker_pid IS NULL
                    AND EXISTS (
                        SELECT 1
                        FROM task_runs newer INDEXED BY idx_runs_task_outcome_id
        ...
        JOIN task_runs r ON r.id = (
              SELECT active.id
              FROM task_runs active INDEXED BY idx_runs_task
              WHERE active.task_id = task.id
...
def load_diagnostic_history(
    ...
    *,
    ...
    if not ids:
        return DiagnosticHistory(events_by_task={}, runs_by_task={})
...
def compute_task_diagnostics(
    ...
    *,
    ...
    out: list[Diagnostic] = []
    terminal_run_diagnostic = _terminal_active_run_diagnostic(task, runs, now_ts)
    if terminal_run_diagnostic is not None:
        out.append(terminal_run_diagnostic)
    for rule in _RULES:
        try:
