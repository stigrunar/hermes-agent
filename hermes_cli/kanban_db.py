...
class Task:
    ...
    def from_row(cls, row: sqlite3.Row) -> "Task":
        ...
        parsed_scope = _json_or(g("mutation_scope"))
        mutation_scope = (
            [str(item) for item in parsed_scope if item]
...
def create_task(
    ...
    if normalized_mutation_scope is not None:
        ...
        if not normalized_mutation_repository:
            raise ValueError(
                "mutation_scope requires mutation_repository or linked project repo")
...
def _admit_task_execution_projection(
    ...
    with _odb.connect_closing() as outcomes_conn:
        ...
        return _odb.admit_execution(outcomes_conn, payload["execution_id"])
    ...
    try:
        ...
        with _odb.connect_closing() as outcomes_conn:
            ...
            if execution is None or execution["state"] in _odb.TERMINAL_EXECUTION_STATES:
                return
            for release in (_odb.release_mutation_lease, _odb.release_resource_lease):
                try:
                    release(outcomes_conn, owner_execution_id=execution_id, reason=reason)
...
def _heartbeat_task_execution_projection(
    ...
    try:
        with _odb.connect_closing() as outcomes_conn:
            _odb.heartbeat_execution(
                outcomes_conn, kanban_execution_id(task_id, board=board))
    except Exception as exc:
        _log.warning(
...
def claim_task(
                ...
                with write_txn(conn):
                    _append_event(conn, task_id, "execution_admission_blocked", {
                        ...
                        "execution_id": kanban_execution_id(
                            task_id, board=projection_board),
                        "counts": exc.counts,
                    })
                return None
            except Exception as exc:
    ...
    with write_txn(conn):
        ...
        if not _parents_satisfied(conn, task_id):
            ...
            if projection_admitted:
                _rollback_task_execution_admission(
                    task_id, board=projection_board, reason="parents_not_done")
            return None
        ...
        if run_id is None:
            ...
            if projection_admitted:
                _rollback_task_execution_admission(
                    task_id, board=projection_board, reason="claim_lost")
            return None
...
def release_stale_claims(
    ...
    for row in stale:
        ...
        termination = (
            ...
            else _terminate_reclaimed_worker(
...
def complete_task(
    ...
    if expected_run_id is None and not force and guard_row and _claim_is_live(guard_row):
...
def request_review(
    ...
    *reviewer's* completion is what cleans the managed scratch workspace up, so
    ...
    if expected_run_id is None and not force and _claim_is_live(guard_row):
        return _ret(
...
def archive_task(conn: sqlite3.Connection, task_id: str, *, signal_fn=None) -> bool:
    ...
    scope_release = _scope_release_result(conn, task_id, previous_run_id)
    if not scope_release.can_release:
        return False
    with write_txn(conn):
        row = conn.execute(
    ...
    if was_running:
        termination = (
            ...
            else _termination_metadata_without_pid_signal(prev_pid, scope_release)
        )
        with write_txn(conn):
