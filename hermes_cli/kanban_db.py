...
def _redact_diagnostic(value: Any) -> Any:
    """Recursively mask secrets before persisting diagnostic-only fields."""
...
class ReviewFinding:
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "ReviewFinding":
        ...
        if len(evidence_refs) != len(raw_refs):
            raise ValueError(
                "review finding evidence_refs must contain non-empty strings"
            )
...
def _dispatch_tick_lock(db_path: Path):
    ...
    *regardless of how the second one got there*.
    ...
    try:
        ...
        acquired = True
        handle = None
    lock_token = _DISPATCH_LOCK_HELD.set(True) if acquired else None
    try:
        yield acquired
    finally:
...
def add_comment(
    ...
    with write_txn(conn):
        ...
        acknowledge_owner_replan_from_comment(
            conn,
            task_id,
            author=author.strip(),
            body=body.strip(),
...
def _dependency_wait_payload(
    ...
    fingerprints = {
        ...
        if (fingerprint := _dependency_fingerprint(conn, dep_id)) is not None
    }
    return {
        "reason": reason,
        "kind": "dependency",
...
def submit_review_verdict(
    ...
    *,
    ...
    typed_findings = validate_review_findings(findings)
    if any(
        finding.classification is ReviewFindingClassification.ACCEPTED_RISK
        for finding in typed_findings
    ):
        raise PermissionError(
            "accepted_risk requires a canonical-default outcome review decision"
        )
    if verdict == "changes_requested" and not any(
        finding.classification is ReviewFindingClassification.BLOCKER
        for finding in typed_findings
    ):
        raise ValueError("changes_requested requires at least one valid typed blocker")
    serialized_findings = [finding.to_mapping() for finding in typed_findings]
    preflight_handoff = _review_handoff_row(conn, review_task_id=review_task_id)
    if (
        preflight_handoff is not None
        and preflight_handoff["state"] == verdict
        and preflight_handoff["verdict"] == verdict
    ):
        return True
    if preflight_handoff is not None:
        _assert_review_budget_allows_verdict(
            conn,
            source_task_id=preflight_handoff["source_task_id"],
            review_task_id=review_task_id,
            verdict=verdict,
        )
...
def _assert_review_budget_allows_verdict(
    conn: sqlite3.Connection,
    *,
    source_task_id: str,
    review_task_id: str,
    ...
    needs_extra_review = prior >= 2 or (prior >= 1 and verdict == "changes_requested")
    if not needs_extra_review:
        return
    last_verdict_id = conn.execute(
    ...
    rows = conn.execute(
        ...
        "AND kind='outcome_review_decision' AND id>? ORDER BY id DESC",
        (source_task_id, last_verdict_id),
    ).fetchall()
    for row in rows:
        try:
        ...
        if (
            ...
            == OutcomeReviewDecision.AUTHORIZE_EXTRA_REVIEW.value
        ):
            return
    raise ValueError(
        "standard review budget exhausted; canonical default must authorize extra review"
    )
...
def apply_outcome_review_decision(
    ...
    *,
    ...
    """Apply a delivery-control decision owned only by canonical default.

    Apart from ``closeout``, decisions are durable authorizations or holds and
    deliberately do not create tasks, links, or status transitions.
    """
    actor = _canonical_assignee(actor_profile)
    if actor != "default":
        raise PermissionError(
            "outcome review control decisions require canonical profile default"
        )
    try:
        typed_decision = OutcomeReviewDecision(decision)
    except (TypeError, ValueError) as exc:
        raise ValueError("invalid outcome review decision") from exc
    typed_findings = validate_review_findings(findings)
    if typed_decision is OutcomeReviewDecision.CLOSEOUT and any(
        finding.classification is ReviewFindingClassification.BLOCKER
        for finding in typed_findings
    ):
        raise ValueError("delivery closeout cannot accept blocking findings")

    criterion = (
        mandatory_criterion_id.strip()
        if isinstance(mandatory_criterion_id, str) and mandatory_criterion_id.strip()
        else None
    )
    if typed_decision is OutcomeReviewDecision.HOLD_CLOSEOUT:
        not_verified = [
            finding
            for finding in typed_findings
            if finding.classification is ReviewFindingClassification.NOT_VERIFIED
        ]
        if criterion is None or not not_verified:
            raise ValueError(
                "hold_closeout requires a mandatory criterion and not_verified finding"
            )
        if any(
            finding.criterion_id is not None and finding.criterion_id != criterion
            for finding in not_verified
        ):
            raise ValueError("not_verified finding criterion does not match hold")
    elif criterion is not None:
        raise ValueError("mandatory_criterion_id is valid only for hold_closeout")

    payload = {
        "actor_profile": actor,
        "decision": typed_decision.value,
        "summary": summary,
        "findings": [finding.to_mapping() for finding in typed_findings],
    ...
    if criterion is not None:
        payload["mandatory_criterion_id"] = criterion

    if typed_decision is OutcomeReviewDecision.CLOSEOUT:
        completed = complete_task(
            conn,
            task_id,
            summary=summary or "delivery closeout authorized by canonical default",
            metadata={"outcome_review_decision": payload},
        )
        if not completed:
            raise ValueError("delivery task is not eligible for closeout")
        return payload
...
class DispatchResult:
    ...
    """Phase-B terminal worker-reap decisions made before maintenance."""
    promoted: int = 0
...
def _terminate_reclaimed_worker(
    ...
    *,
    ...
    # this dispatcher could have launched one.
    if conn is not None and task_id is not None and run_id is not None:
        ...
        launch_acknowledged = getattr(resolved_scope, "launch_acknowledged", None)
        ...
        if scope_state == "not-found" and launch_mode == "systemd-user-scope":
            if launch_acknowledged is True:
                ...
                return info
            if not pid_known_dead and _pid_alive(pid):
                info["scope_unknown"] = True
...
def _resolve_systemd_user_manager_target(
    ...
    *,
    ...
    """Accept only the current real/effective UID and its private user bus."""
    if not (platform if platform is not None else sys.platform).startswith("linux"):
        return None
    if manager_kind != _SYSTEMD_USER_MANAGER_KIND:
        return None
    if isinstance(manager_uid, bool) or not isinstance(manager_uid, int) or manager_uid < 0:
        return None
    getuid = getattr(os, "getuid", None)
    geteuid = getattr(os, "geteuid", None)
    if getuid is None or geteuid is None:
        return None
    uid = int(getuid()) if real_uid is None else int(real_uid)
    euid = int(geteuid()) if effective_uid is None else int(effective_uid)
    if manager_uid != uid or manager_uid != euid:
        return None
    root = Path(runtime_root) if runtime_root is not None else _SYSTEMD_USER_RUNTIME_ROOT
    runtime_dir = root / str(manager_uid)
    bus_path = runtime_dir / "bus"
    inspect = lstat_fn or os.lstat
    try:
        runtime_stat, bus_stat = inspect(runtime_dir), inspect(bus_path)
    except (OSError, TypeError, ValueError):
        return None
    if (
        not stat.S_ISDIR(runtime_stat.st_mode)
        or runtime_stat.st_uid != manager_uid
        or stat.S_IMODE(runtime_stat.st_mode) & 0o077
        or not stat.S_ISSOCK(bus_stat.st_mode)
        or bus_stat.st_uid != manager_uid
    ):
        return None
    return _SystemdUserManagerTarget(
        _SYSTEMD_USER_MANAGER_KIND, manager_uid, runtime_dir, bus_path,
    )
...
def _current_systemd_user_manager_target() -> Optional[_SystemdUserManagerTarget]:
    ...
    return (
        _resolve_systemd_user_manager_target(
            _SYSTEMD_USER_MANAGER_KIND, int(getuid())
        )
        if getuid is not None else None
    )


def _systemd_user_manager_environment(target: _SystemdUserManagerTarget) -> dict[str, str]:
    return {
...
def _systemd_user_manager_target_for_cgroup(
    ...
    if _systemd_service_manager(cgroup_path) != "user":
        return None
    match = re.search(r"/user@(\d+)\.service(?:/|$)", cgroup_path or "")
    if match is None:
        return None
    return _resolve_systemd_user_manager_target(
        _SYSTEMD_USER_MANAGER_KIND, int(match.group(1)),
    )


def _systemd_run_major_version(runner: str, *, run_fn=None) -> Optional[int]:
    run = run_fn or subprocess.run
    try:
        result = run(
            [runner, "--version"], stdout=subprocess.PIPE,
            stderr=subprocess.DEVNULL, text=True,
            timeout=_SYSTEMD_PROBE_TIMEOUT_SECONDS, check=False,
        )
    except (OSError, subprocess.SubprocessError, TimeoutError):
        return None
...
def _systemd_user_scope_available(
    runner: str, *, manager_target=None, run_fn=None, systemctl=None,
) -> bool:
    version = _systemd_run_major_version(runner, run_fn=run_fn)
    return bool(
        version is not None and version >= _SYSTEMD_SCOPE_MIN_VERSION
        and _systemd_user_manager_reachable(
            manager_target=manager_target, systemctl=systemctl, run_fn=run_fn,
        )
    )
...
def _worker_scope_unit_name(
    ...
    *, board: Optional[str] = None,
    ...
    identity = os.path.normcase(str(path.expanduser().resolve(strict=False)))
    digest = hashlib.blake2s(
        f"v1\0{identity}\0{task_id}\0{int(run_id)}".encode("utf-8"),
        digest_size=16,
    ).hexdigest()
...
def _valid_worker_scope_unit(unit: str) -> bool:
    return bool(
        isinstance(unit, str) and len(unit) <= 255
        and (
...
def _systemd_worker_scope_argv(
    ...
    *,
    ...
    """Use an authenticated sibling user scope only for user services."""
    ...
    if target is None or runner is None or task.current_run_id is None:
        return cmd
    if (
    ...
    ready = (
        ...
        if user_manager_ready is None else bool(user_manager_ready)
    )
    if not ready:
        return cmd
    try:
        unit = _worker_scope_unit_name(
            task.id, int(task.current_run_id), board=board, db_path=db_path,
        )
    except (OSError, TypeError, ValueError):
        return cmd
    return [
        runner, "--user", "--scope", "--quiet", "--collect", f"--unit={unit}",
        "--", *cmd,
    ]


def _systemd_user_scope_state(
    unit: str, *, manager_target=None, systemctl=None, run_fn=None,
) -> str:
    ...
    try:
        result = run(
            [controller, "--user", "show", unit, "--property=LoadState",
    ...
    if result.returncode != 0:
        return "unknown"
    props = dict(
        line.split("=", 1) for line in (result.stdout or "").splitlines()
        if "=" in line
    )
    ...
    if props.get("ActiveState") in {"inactive", "failed"}:
        return "inactive"
    return "active" if props.get("ActiveState") else "unknown"
...
def _stop_systemd_user_scope(
    ...
    try:
        run(
            [controller, "--user", "stop", unit], stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL, timeout=7.0, check=False,
            env=_systemd_user_manager_environment(target),
        )
    except (OSError, subprocess.SubprocessError, TimeoutError):
        pass
    for _ in range(10):
        state = _systemd_user_scope_state(
            unit, manager_target=target, systemctl=controller, run_fn=run,
        )
        if state == "not-found":
            return True
        if state == "unknown":
...
class _WorkerScopeStatus:
    """Resolved worker-scope state and the manager evidence behind it."""
...
def _scope_status_fields(
    scope: Any,
) -> tuple[Any, str, Any, str, Optional[bool]]:
    """Read modern scope receipts and legacy two-tuple test seams alike."""
    if isinstance(scope, _WorkerScopeStatus):
        return (
            scope.unit,
            scope.state,
            scope.manager_target,
            scope.launch_mode,
            scope.launch_acknowledged,
        )
    try:
        unit, state = scope
    except (TypeError, ValueError):
...
def _worker_scope_state(
    ...
    *,
    ...
    """Resolve only the exact scope and manager recorded for this run."""
    if run_id is None:
    ...
    try:
        ...
        payload = json.loads(event["payload"]) if event and event["payload"] else None
    except (sqlite3.Error, TypeError, ValueError, json.JSONDecodeError):
        return _WorkerScopeStatus(None, "unknown", None, "invalid")
    if not isinstance(payload, dict):
        return _WorkerScopeStatus(None, "not-applicable", None, "untracked")
    launch_mode = payload.get("launch_mode", "direct")
    if launch_mode == "direct" and not any(
        key in payload
        for key in ("scope_unit", "manager_kind", "manager_uid", "launch_acknowledged")
    ):
        return _WorkerScopeStatus(None, "not-applicable", None, "direct")
    if launch_mode != "systemd-user-scope":
        return _WorkerScopeStatus(None, "unknown", None, "invalid")
    ack = payload.get("launch_acknowledged")
    ...
    target = _resolve_systemd_user_manager_target(
        payload.get("manager_kind"), payload.get("manager_uid"),
    )
    if target is None:
        return _WorkerScopeStatus(None, "unknown", None, launch_mode, ack)
    resolved_db = (
        _canonical_db_path(db_path)
        if db_path is not None else _connection_main_db_path(conn)
    )
    if resolved_db is None:
        return _WorkerScopeStatus(None, "unknown", None, launch_mode, ack)
    expected_unit = _worker_scope_unit_name(
        task_id, int(run_id), db_path=resolved_db,
    )
    unit = payload.get("scope_unit")
    if unit != expected_unit or not _valid_worker_scope_unit(unit):
        return _WorkerScopeStatus(None, "unknown", None, launch_mode, ack)
    state = _systemd_user_scope_state(unit, manager_target=target)
    return _WorkerScopeStatus(unit, state, target, launch_mode, ack)
...
def _board_slug_for_connection(conn: sqlite3.Connection) -> str:
    """Return the canonical board slug for an already-open Kanban DB."""
    path = _connection_main_db_path(conn)
    if path is not None and path.name == "kanban.db":
        parent = path.parent
        if parent.parent.name == "boards" and parent.name:
            return _normalize_board_slug(parent.name) or DEFAULT_BOARD
    return DEFAULT_BOARD
...
def _owner_replan_route(
    ...
    """Resolve one durable default-owner route from existing notify provenance."""
    row = conn.execute(
        "SELECT platform, chat_id, chat_type, thread_id, user_id, notifier_profile, "
        "session_key FROM kanban_notify_subs WHERE task_id=? "
...
def _owner_replan_project_identity(task: Mapping[str, Any], board: str) -> tuple[str, str]:
    """Return a durable project/routing identity without inventing a Project row."""
    project_id = str(task.get("project_id") or "").strip()
    if project_id:
        return project_id, "project_id"
    tenant = str(task.get("tenant") or "").strip()
    if tenant:
...
def _owner_replan_repo_suppression(task: Mapping[str, Any]) -> Optional[str]:
    """Return why repo canon makes this revision non-current/complete, if proven."""
    canon_path = resolve_canon_path(task)
    if not canon_path:
        return None
    contract_id, revision = resolve_contract_identity(task)
    try:
        state = read_repo_execution_states(canon_path).get(contract_id)
    except (OSError, UnicodeError):
        return None
    if not state:
        return None
...
def _owner_replan_active_successor(
    ...
    """Return an active same-contract replacement task id, if one is explicit."""
    ...
    for row in rows:
        ...
        if continuation and continuation.group(1).strip() == task_id:
            return str(row["id"])
        # Explicit contract ids provide a second authoritative successor seam.
        # Legacy fallback contracts remain task-local and are never joined by
        # title/prose similarity.
        if not contract_id.startswith("task:"):
            other_contract, other_revision = resolve_contract_identity(candidate)
...
def _find_owner_replan_control(
    conn: sqlite3.Connection,
    task_id: str,
    fingerprint: str,
) -> Optional[str]:
    rows = conn.execute(
        ...
        "AND kind IN (?,?,?,?,?) ORDER BY id DESC",
        (
            task_id,
            "owner_replan_wake_claimed",
            "owner_replan_delivered",
            "owner_replan_acknowledged",
            "owner_replan_failed",
...
def _ensure_owner_replan_event(
    ...
    *,
    ...
    """Append one durable owner-replan intent after terminal non-retry state is set.

    Caller holds the write transaction. This helper never changes task
    dispatchability; it only records a one-shot control intent in task_events.
    """
    task_id = str(task_snapshot.get("id") or "")
    if not task_id or terminal_run_id is None:
        return None
    if str(task_snapshot.get("status") or "") != "running":
        return None
    if int(task_snapshot.get("current_run_id") or 0) != int(terminal_run_id):
        return None
    if is_non_executable_hygiene(task_snapshot.get("hygiene_class")):
        return None
    if str(task_snapshot.get("superseded_by") or "").strip():
        return None
    if str(end_reason or "").casefold() == "needs_user_decision":
        return None
    repo_suppression = _owner_replan_repo_suppression(task_snapshot)
    if repo_suppression:
        return None
    ...
    if route is None:
        # No bound default-owner continuation surface: do not create a durable
        # event that no existing controller can consume.
        return None
    board = _board_slug_for_connection(conn)
    project, project_source = _owner_replan_project_identity(task_snapshot, board)
    contract_id, revision = resolve_contract_identity(task_snapshot)
    payload: dict[str, Any] = {
        "event_type": _OWNER_REPLAN_EVENT_KIND,
        "owner": "default",
        "project": project,
        ...
        "worktree": str(task_snapshot.get("workspace_path") or ""),
        "branch": str(task_snapshot.get("branch_name") or ""),
...
def claim_owner_replan_for_route(
    ...
    *,
    ...
    """Resolve or atomically claim one pending owner-replan for its exact route.

    ``claim=False`` is a read-only peek used by the notifier before terminal
    text delivery. The durable at-most-once claim is written only immediately
    before the actual owner wake attempt, so a failed text ping cannot consume
    the continuation. A failed owner wake is terminalized as
    owner_replan_failed/manual rather than reclaimed on a later tick.
    """
    ...
    with txn:
        ...
        for row in rows:
            ...
            if not fingerprint:
                continue
            control = _find_owner_replan_control(conn, task_id, fingerprint)
            if control:
                # A notifier crash after the durable claim but before a
                # delivered/failed receipt must never trigger a second owner
                # wake. A later read-only peek surfaces the interrupted claim so
                # the notifier can convert it to one manual failure receipt.
            ...
            suppression = None
            if is_non_executable_hygiene(task.hygiene_class) or task.superseded_by:
                suppression = "superseded_or_obsolete"
            elif _owner_replan_repo_suppression(task_map):
                suppression = _owner_replan_repo_suppression(task_map)
            else:
                successor = _owner_replan_active_successor(conn, task_map)
                if successor:
                    suppression = f"active_successor:{successor}"
            if suppression:
                if claim:
                    _append_diagnostic_event(
                        conn,
...
def mark_owner_replan_delivered(
    ...
    *,
    ...
    with write_txn(conn):
        if _find_owner_replan_control(conn, task_id, fingerprint) in {
        ...
        _append_diagnostic_event(
            ...
            {
                "fingerprint": fingerprint,
                "replan_event_id": int(replan_event_id),
                "owner": "default",
            },
        )
    return True
...
def mark_owner_replan_failed(
    ...
    *,
    ...
    with write_txn(conn):
        existing = _find_owner_replan_control(conn, task_id, fingerprint)
        if existing in {"owner_replan_failed", "owner_replan_delivered", "owner_replan_acknowledged"}:
            return False
        _append_diagnostic_event(
            conn,
            task_id,
...
def acknowledge_owner_replan_from_comment(
    ...
    *,
    ...
    """Turn an exact default-owner ack comment into a durable ledger receipt."""
    ...
    for row in rows:
        ...
        if not isinstance(payload, dict) or payload.get("fingerprint") != fingerprint:
            continue
        if _find_owner_replan_control(conn, task_id, fingerprint) == "owner_replan_acknowledged":
            return False
        _append_diagnostic_event(
            conn,
            task_id,
...
def _record_iteration_exhaustion(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        # The worker revision is already terminal/non-retryable above. Record a
        # separate owner control intent; never make this revision runnable again.
...
def _set_worker_pid(
    ...
    *,
    ...
    """CAS-record the PID and the private managed-scope launch receipt."""
    ...
    with write_txn(conn):
        ...
        if scoped_launch:
            ...
            elif launch_handle.manager_kind is not None or launch_handle.manager_uid is not None:
                payload.pop("scope_unit", None)
            if type(launch_handle.launch_acknowledged) is bool:
                payload["launch_acknowledged"] = launch_handle.launch_acknowledged
            elif launch_handle.launch_acknowledged is not None:
                payload.pop("scope_unit", None)
...
def dispatch_once(
    ...
    *,
    ...
    """
    if dry_run:
        # Preview never enters the writer lock: opening its sidecar would be a
        # filesystem effect, and no mutation below needs single-writer
        # serialization. The active connection remains the read source.
...
def _preview_dispatch_maintenance(
    conn: sqlite3.Connection,
    *,
    stale_timeout_seconds: int = 0,
    failure_limit: int = DEFAULT_SPAWN_FAILURE_LIMIT,
) -> DispatchResult:
    """Describe maintenance candidates without touching DB or processes.

    The maintenance helpers intentionally own their write transactions and
    some of them can signal workers.  A dry-run must not call those helpers at
    all, even with ``process_effects=False``: that mode is useful to their
    callers, but is not a sufficient whole-tick no-write contract.
    """
    ...
    # PID-less expiry is deterministic.  PID-bearing rows stay out of the
    # preview count because deciding ownership would require a process census.
    result.reclaimed = sum(1 for row in expired if row["worker_pid"] is None)
    ...
    if stale_timeout_seconds > 0:
        ...
        for row in rows:
            if row["started_at"] is None:
                continue
            if now - int(row["started_at"]) < stale_timeout_seconds:
                continue
            heartbeat_age = (
                now - int(row["last_heartbeat_at"])
                if row["last_heartbeat_at"] is not None else None
            )
            if heartbeat_age is not None and heartbeat_age < _STALE_HEARTBEAT_GAP_SECONDS:
                continue
            if row["worker_pid"] is None:
                result.stale.append(row["id"])

    timed_out = conn.execute(
        "SELECT t.id, COALESCE(r.started_at, t.started_at) AS started_at, "
        "t.max_runtime_seconds FROM tasks t "
        "LEFT JOIN task_runs r ON r.id=t.current_run_id "
        "WHERE t.status='running' AND t.max_runtime_seconds IS NOT NULL"
    ).fetchall()
    for row in timed_out:
        if row["started_at"] is not None and (
            now - int(row["started_at"]) >= int(row["max_runtime_seconds"])
        ):
            result.timed_out.append(row["id"])
    ...
    for row in candidates:
        ...
        if row["status"] == "blocked":
            limit = int(row["max_retries"] or failure_limit or DEFAULT_FAILURE_LIMIT)
...
def _dispatch_once_locked(
    ...
    *,
    ...
    """
    if dry_run:
        # Strict preview: no transaction-owning maintenance helper and no
        # waitpid/process-tree census. The rest of this function only issues
        # reads or records would-be spawn decisions when dry_run is true.
        result = _preview_dispatch_maintenance(
            conn,
            stale_timeout_seconds=stale_timeout_seconds,
            failure_limit=failure_limit,
        )
        ...
        result = DispatchResult()
        # Phase-B terminal intents must be reconciled before stale/crash
        # maintenance can reclaim the same run or spawn a replacement.
...
def _default_spawn(
    ...
    *,
    ...
    # Worker transcripts belong to the board, not interactive session pickers.
    # Tag them before profile/runtime policy resolution so every spawn path
    # persists the upstream session-source contract.
    ...
    if task.goal_mode:
        ...
        cmd.append("-Q")
    worker_cmd = cmd
    db_path = _canonical_db_path(kanban_db_path(board=board))
    ...
    if cmd is not worker_cmd:
        delimiter = cmd.index("--")
        scope_unit = next(
            part.removeprefix("--unit=")
            for part in cmd[:delimiter]
            if part.startswith("--unit=")
        )
        if scope_manager_target is None:
...
def list_notify_subs(
    ...
    *,
    ...
    available to explicit migration/inspection callers for legacy rows created
    before profile stamping; gateway delivery remains fail-closed and leaves it
    false.
    """
