...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_outcome_create.add_argument("--acceptance", action="append", default=None, dest="frozen_acceptance")
    ...
    p_outcome_update.add_argument("--acceptance", action="append", default=None, dest="frozen_acceptance")
    ...
    p_exec_create = sub.add_parser("execution-create", help="Register one root-shared Outcome execution")
    p_exec_create.add_argument("project", help="Project id or slug")
    p_exec_create.add_argument("outcome", help="Outcome id or key")
    p_exec_create.add_argument("--mode", required=True, choices=["direct_codex", "kanban", "external"])
    ...
    p_exec_terminal.add_argument("--state", required=True, choices=["completed", "cancelled", "failed"])
    p_exec_terminal.add_argument("--receipt", default=None, dest="receipt_uri")
    p_exec_terminal.add_argument("--reason", default=None)

    p_direct_codex = sub.add_parser(
        "direct-codex-run",
        help="Register, admit, heartbeat and terminalize one bounded direct Codex execution",
    )
    p_direct_codex.add_argument("project", help="Project id or slug")
    p_direct_codex.add_argument("outcome", help="Outcome id or key")
    p_direct_codex.add_argument("--repo", required=True, help="Clean isolated repo/worktree path")
    ...
    p_direct_codex.add_argument("--lane", default=None, dest="conversation_lane_id")
    p_direct_codex.add_argument("--resource", action="append", default=None, dest="resources")
    ...
    p_resource_request = sub.add_parser("resource-request", help="Request/acquire a shared execution resource")
    ...
    p_resource_release = sub.add_parser("resource-release", help="Release shared resources held/waited by an execution")
    ...
    p_materialize = sub.add_parser("materialize-status", help="Write docs/outcomes/<OUTCOME>/00-status.md")
    p_materialize.add_argument("project", help="Project id or slug")
    p_materialize.add_argument("outcome", help="Outcome id or key")
    p_materialize.add_argument("--repo", default=None)

    p_tg = sub.add_parser("telegram-provision", help="Legacy manual topic provisioning (prefer telegram-sync)")
    p_tg.add_argument("project", help="Project id or slug")
    p_tg.add_argument("chat_id", help="Existing Telegram forum supergroup id")
    p_tg.add_argument("--control", default=None, metavar="NAME")
    p_tg.add_argument("--topic", action="append", default=[], metavar="NAME[=OUTCOME]")
    p_tg_sync = sub.add_parser(
        "telegram-sync", help="Sync one registry-declared Telegram project group projection"
    )
    p_tg_sync.add_argument("project", help="Project id or slug")
    p_tg_sync.add_argument("--registry", required=True, metavar="PATH")
    p_tg_sync.add_argument("--dry-run", action="store_true")
    p_tg_sync.add_argument("--json", action="store_true", dest="as_json")
...
def _cmd_snapshot(args, _conn, proj) -> int:
    ...
    if not outcomes:
        ...
        for outcome in outcomes:
            active = [
                lease for lease in snapshot["active_mutation_leases"]
...
def _cmd_execution_admit(args, _conn, proj) -> int:
    ...
    with odb.connect_closing() as oc:
        ...
        try:
            admitted = odb.admit_execution(oc, execution["execution_id"], require_feature_gate=True)
        except odb.ExecutionAdmissionBlocked as exc:
            current = odb.get_execution(oc, execution["execution_id"])
            print(json.dumps({"execution": current, "admitted": False, "reason": exc.reason, "counts": exc.counts}, ensure_ascii=False, sort_keys=True))
            return 3
...
def _cmd_execution_terminal(args, _conn, proj) -> int:
    ...
    with odb.connect_closing() as oc:
        ...
        ok = odb.terminalize_execution(
            oc, execution["execution_id"], state=args.state,
            receipt_uri=args.receipt_uri, reason=args.reason,
        )
        current = odb.get_execution(oc, execution["execution_id"])
    print(json.dumps({"ok": ok, "execution": current}, ensure_ascii=False, sort_keys=True))
    return 0 if ok else 1
...
def _cmd_direct_codex_run(args, _conn, proj) -> int:
    ...
    from hermes_cli.direct_codex_execution import (
        DirectCodexExecutionError, canonical_repo_identity, current_base_ref,
    ...
    with odb.connect_closing() as oc:
        ...
        try:
            ...
            current = odb.get_execution(oc, execution_id)
            print(json.dumps({
                "ok": False, "execution": current, "reason": exc.reason,
                "counts": exc.counts, "durable_wait_required": exc.reason == "waiting_resource",
            }, ensure_ascii=False, sort_keys=True))
            return 3
        except DirectCodexExecutionError as exc:
            current = odb.get_execution(oc, execution_id)
            print(json.dumps({"ok": False, "execution": current, "error": str(exc)}, ensure_ascii=False, sort_keys=True))
            return 2
...
def _cmd_resource_request(args, _conn, proj) -> int:
    ...
    with odb.connect_closing() as oc:
        ...
        lease = odb.request_resource_lease(
            oc, resource_key=args.resource, owner_execution_id=execution["execution_id"],
            purpose=args.purpose, capacity=args.capacity,
        )
...
def _cmd_resource_release(args, _conn, proj) -> int:
    ...
    with odb.connect_closing() as oc:
        ...
        result = odb.release_resource_lease(
            oc, owner_execution_id=execution["execution_id"], reason=args.reason,
            stale=bool(args.stale), verified_dead=bool(args.verified_dead),
        )
    print(json.dumps(result, ensure_ascii=False, sort_keys=True))
    return 0
...
def _cmd_materialize_status(args, _conn, proj) -> int:
    ...
    target = materialize_status(
        project_id=proj.id, project_name=proj.name, outcome_id=outcome.id,
        repo=Path(repo),
    )
    print(target)
    return 0
...
def _cmd_telegram_provision(args, _conn, proj) -> int:
    from hermes_cli.project_forum import TopicSpec, provision_telegram_topics_with_configured_bot
    ...
    for result in results:
        ...
        print(f"{verb} Telegram topic {lane.get('label')!r} thread={lane.get('thread_id')} -> project={proj.id}{outcome}")
    return 0
...
def _cmd_telegram_sync(args, _conn, proj) -> int:
    from hermes_cli.project_telegram import (
        load_project_registry, sync_telegram_project,
        sync_telegram_project_with_configured_bot,
    )
    try:
        spec = load_project_registry(
            args.registry, project_ident=proj.slug, canonical_project_id=proj.id,
        )
        result = asyncio.run((sync_telegram_project if args.dry_run
                              else sync_telegram_project_with_configured_bot)(
                                  spec=spec, canonical_project_id=proj.id, dry_run=args.dry_run,
                              ))
    except (ValueError, RuntimeError) as exc:
        print(f"project: {exc}", file=sys.stderr)
        return 2
    ...
    print(f"{mode}: {proj.slug} -> Telegram {result['chat_id']} ({len(result['actions'])} action(s))")
    for action in result["actions"]:
        detail = " ".join(f"{key}={value}" for key, value in action.items() if key != "action" and value is not None)
    ...
    if result["registry_updates"]:
        ...
        print(json.dumps(result["registry_updates"], ensure_ascii=False, sort_keys=True), file=sys.stderr)
    return 0
