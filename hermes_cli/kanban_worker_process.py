...
def capture_process_tree(
    ...
    if root_state == "gone":
        # The root may exit after terminal request while captured detached
        # descendants remain.  Preserve the durable set for exact checks.
        return TreeCapture("captured", tuple(prior.values()))
...
def signal_exact(
    ...
    *,
    ...
    if state != "alive":
        return state
    sender = signal_fn or os.kill
    try:
        sender(identity.pid, sig)
