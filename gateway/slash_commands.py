...
class GatewaySlashCommandsMixin(
    ...
    async def _handle_voice_command(self, event: MessageEvent) -> str:
        ...
        # message, so this handler returns no duplicate response.
        if not args and event.source.platform == Platform.TELEGRAM and adapter is not None:
            launcher = getattr(adapter, "send_voice_mini_app_launcher", None)
            if callable(launcher):
                try:
                    from gateway.voice_topic_binding import mint_voice_launch

                    launch_url = await mint_voice_launch(self, event, adapter)
                except Exception as exc:
